...
<style>
    ...
    .metric-label { color: #7faec8; font-size: 13px; margin-bottom: 4px; }
...
CHART_BASE = dict(
    ...
    font=dict(color="#c9d6e3", family="Inter, sans-serif", size=13),
    legend=dict(bgcolor="rgba(0,0,0,0)", bordercolor="#1f3a5f",
                borderwidth=1, font=dict(size=13)),
    margin=dict(l=70, r=40, t=55, b=65),
)
AXIS_STYLE = dict(gridcolor="#1a3050", linecolor="#2a4a6f",
                  zerolinecolor="#1f3a5f", tickfont=dict(size=12))
...
fig_map.update_layout(map_style="open-street-map", height=480,
...
# ── Tab 1: Proper oceanographic profile ───────────────────────────────────────
with tab1:
    ...
    if x_spread < 0.1:
        x_spread = max(x_spread, 0.5 if param_col == "salinity" else 1.0)
    x_pad = x_spread * 0.15
    ...
    for region in regions:
        ...
        # Shaded IQR band — depth on X, parameter on Y
        fig1.add_trace(go.Scatter(
            x=list(depths) + list(depths[::-1]),
            y=list(stats["p75_s"]) + list(stats["p25_s"][::-1]),
            fill="toself",
            fillcolor=hex_to_rgba(col_hex, 0.18),
            line=dict(width=0),
            showlegend=False,
            hoverinfo="skip",
        ))
        # Mean line — depth on X, parameter on Y
        fig1.add_trace(go.Scatter(
            x=depths,
            y=stats["mean_s"],
            mode="lines",
    ...
    fig1.update_layout(
        **CHART_BASE,
        height=520,
        title=dict(
            text=f"Mean {parameter} Profile with Depth",
        ...
        yaxis=dict(
            **AXIS_STYLE,
            title=dict(text=f"{parameter} ({param_unit})", font=dict(size=14)),
            range=[x_min - x_pad, x_max + x_pad],
        ),
    )
    st.plotly_chart(fig1, use_container_width=True)
    st.caption("X = Depth (m, surface→deep) | Y = " + parameter + " | Shaded band = IQR (25th–75th percentile)")
...
with tab2:
    ...
    fig2.update_layout(
        **CHART_BASE,
        height=520,
        title=dict(
            text=f"{parameter} Distribution — Arabian Sea vs Bay of Bengal",
            font=dict(size=16, color="#e0eaf5")
        ),
        yaxis=dict(
            **AXIS_STYLE,
...
# ── Tab 3: Yearly trend — zoomed Y axis ───────────────────────────────────────
with tab3:
    yearly = (
        filtered_df.groupby(["year", "region"])[param_col]
        .mean().reset_index()
        .rename(columns={param_col: "mean_val"})
    )

    y_min3 = yearly["mean_val"].min()
    y_max3 = yearly["mean_val"].max()
    spread = y_max3 - y_min3
    # Give at least 10x the spread as padding so lines don't look flat
    ...
    for region in sorted(yearly["region"].unique()):
        ...
        fig3.add_trace(go.Scatter(
            ...
            marker=dict(size=7, color=col_hex,
    ...
    fig3.update_layout(
        **CHART_BASE,
        height=480,
        title=dict(
            text=f"Mean {parameter} per Year by Region",
            font=dict(size=16, color="#e0eaf5")
        ),
        xaxis=dict(**AXIS_STYLE,
                   title=dict(text="Year", font=dict(size=14))),
        yaxis=dict(**AXIS_STYLE,
                   title=dict(text=f"Mean {parameter} ({param_unit})", font=dict(size=14)),
...
st.caption(f"Showing {min(1000, len(filtered_df)):,} of {len(filtered_df):,} filtered records")
st.dataframe(
